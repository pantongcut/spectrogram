...
pub fn detect_segments(
    ...
    if spectrogram_flat.is_empty() || num_cols == 0 {
        return Vec::new();
    }

    let num_rows = spectrogram_flat.len() / num_cols;
    if num_rows == 0 {
        return Vec::new();
    }

    // Step 1: Scan for active frames
    let mut active_frames = vec![false; num_rows];
    for r in 0..num_rows {
        let row_start = r * num_cols;
        let row_end = row_start + num_cols;
        for i in row_start..row_end {
            if spectrogram_flat[i] >= threshold_db {
                active_frames[r] = true;
                break;
            }
        }
    }
    ...
    for (i, &is_active) in active_frames.iter().enumerate() {
        if is_active {
            if !in_segment {
                in_segment = true;
                start_frame = i;
            ...
            if in_segment {
                in_segment = false;
                segments.push((start_frame, i - 1));
            }
        }
    }
    if in_segment {
        segments.push((start_frame, num_rows - 1));
    }
    ...
    // Step 3: Apply padding and merge
    let time_per_frame = hop_size as f32 / sample_rate;
    let padding_frames = (padding_ms / 1000.0 / time_per_frame).ceil() as usize;

    let mut merged: Vec<(usize, usize)> = Vec::new();

    // Apply padding to first segment
    let mut curr_start = segments[0].0.saturating_sub(padding_frames);
    let mut curr_end = std::cmp::min(segments[0].1 + padding_frames, num_rows - 1);

    for i in 1..segments.len() {
        let next_start = segments[i].0.saturating_sub(padding_frames);
        let next_end = std::cmp::min(segments[i].1 + padding_frames, num_rows - 1);
    ...
    // Step 4: Convert to seconds and flatten
    let mut result = Vec::with_capacity(merged.len() * 2);
    for (s, e) in merged {
        let t_start = s as f32 * time_per_frame;
        let t_end = (e as f32 * time_per_frame) + time_per_frame;
        result.push(t_start);
        result.push(t_end);
    }

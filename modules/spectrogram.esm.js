...
class h extends s {
    ...
    async getFrequencies(t, isRetry = false) {
        if (!this.options || !t || this.isDestroyed) return;
        ...
        if (!this.wrapper || !this._wasmEngine || this.isDestroyed) {
        return null;
    }
        ...
        for (let e = 0; e < i; e++) {
            ...
            try {
             ...
             if (wasmError.message && wasmError.message.includes('aliasing')) {
                ...
                console.error('[Spectrogram] WASM aliasing error:', wasmError);
                this._reinitWasmEngine();
            }
            return null;
        }

        // [NEW] 如果計算完發現已銷毀，立即釋放 WASM 返回的巨大數組
        if (this.isDestroyed) {
            fullU8Spectrum = null; 
            return null;
        }
